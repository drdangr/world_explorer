...
- [x] Вынести историю путешествий в отдельный JSON-файл (связка `worldId` + `characterId`), разгрузив `data/characters.json`.
- [ ] контекстное меню дня ноды в графе (переместиться, связать выделенные, редактировать локацию, удалить, импорт, экспорт, дедупликация)
...
- [ ] Редактирование сообщений в чате: кнопка удаления, in-line редактировани с обновлением соотвествующего JSON файла
