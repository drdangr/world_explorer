...
4. **Инициализация сессии**: игрок запрашивает вступительное описание, ГМ описывает центральную локацию и предлагает стартовые пути.
5. **Исследование**:
   ...
   - ГМ возвращает нарратив, доступные выходы, обновлённые локации, предметы и подсказки.
6. **Сохранение состояния**: после каждого хода обновляются файлы JSON, история сессий и инвентарь персонажа.
...
interface LocationNode {
  id: LocationId;
  locationName: string;
  description: string; // появляется при первом визите
  discovered: boolean;
  items: Item[];
  connections: Record<LocationId, ConnectionMeta>;
}

interface ConnectionMeta {
  id: string;
  label?: string; // подсказка, направление
  bidirectional: boolean; // по умолчанию true
}
...
- **Контекст запроса**: сеттинг/атмосфера/жанр мира, данные персонажа, текущая локация, известные связи и до 12 последних реплик истории.
- **Ответ LLM**: строгое JSON‑описание (`narration`, `playerLocation`, `discoveries`, `inventory`, `suggestions`).
- **Обработка**: `applyGameTurn` нормализует имена локаций, синхронизирует предметы (с сохранением идентификаторов), добавляет связи и обновляет историю.
- **Провайдеры**:
