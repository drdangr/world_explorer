...
- **Компонент**: `GraphPanel` (`src/features/dashboard/components/GraphPanel.tsx`). Рендерит граф локаций на базе React Flow с кастомными нодами (`CircleNode`) и рёбрами (`CircleEdge`).
- **Режимы раскладки**:
  - `entry`: центрируется на стартовой локации мира, формирует радиальную «звёздочку» вокруг неё.
  - `player`: центрируется на текущей локации игрока; остальные узлы выстраиваются относительно неё.
- **Движок раскладки**: `GraphLayoutEngine` (`src/features/dashboard/utils/graphLayout.ts`) — адаптация d3-force, объединяющая силы отталкивания/притяжения, радиальное смещение от центральной ноды и столкновения. Обновляет позиции через `requestAnimationFrame`, обеспечивает плавную анимацию после перетаскивания.
- **Сохранение состояния**:
  - Для каждого мира и режима кэшируются ручные координаты нод и текущий viewport (позиция камеры + zoom).
  - При переключении режима/мира граф стартует из последнего сохранённого состояния, без скачков и задержек.
  - Позиции фиксируются после завершения симуляции и окончания перетаскивания пользователем; viewport — после `fitView` и событий `onMoveEnd`.
- **Управление камерой**:
  - Кнопка с иконкой рамки («Fit») запускает `fitView` с расширенным паддингом и учитывает скрытые ноды, гарантируя попадание всей схемы в область просмотра.
  - Кнопка с иконкой мишени («Center») плавно центрирует текущую ключевую ноду (входную или позицию игрока) без изменения масштаба.
- **Миникарта**: встроенный `MiniMap` от React Flow, стилизован под темную тему; отражает актуальные позиции, zoom и цветовое кодирование статусов (`entry` / `player`).
- **Фокус и управление**:
  - `fitView` используется как fallback, если нет сохранённого viewport.
  - Диапазоны zoom ограничены `0.08–2`, узлы перетаскиваемы, соединения недоступны для редактирования из UI.
...
- Редактирование локаций в панели графа: добавление связей, названия, описания локаций.
...
- Миграция хранилища на Supabase (PostgreSQL + Storage) с синхронизацией в реальном времени — запланирована после финализации модели данных.
